...
void Sender_FromLowerLayer_SR(struct packet *pkt){
    ...
    // 数据包不存在被篡改, 考虑到SR的确认特性，需要打上标记，说明已经收到
    if(hash_cal == Hashed_Val && Part_SeqID > Moving_Window_Left_Index){
        rdt_sender_ackRecord[Part_SeqID] = true;
        
        // 举个例子 window大小10， Moving_Window_Left_Index = 0, Moving_Window_Right_Index = 9
        // 对应的Packet ID = 1 10
        for (unsigned int i = Moving_Window_Left_Index; i <= Moving_Window_Right_Index; i++)
        {
            unsigned int check_seq_ID = i + 1;
            
            if(!rdt_sender_ackRecord.count(check_seq_ID)){
            
                //printf("mov windows fail for %u\n", check_seq_ID);
                rdt_sender_mutex.unlock();
                return;
            }
        }
        ...
        Moving_Window_Left_Index = Moving_Window_Right_Index + 1;
        Moving_Window_Right_Index = Moving_Window_Left_Index + Moving_Window_Size - 1;

        //printf("index [%u, %u ]\n", Moving_Window_Left_Index, Moving_Window_Right_Index);
        for(unsigned int i = Moving_Window_Left_Index; i <= Moving_Window_Right_Index; i++){
            // 变量 i 非法，不访问直接终止循环    
            if(i >= rdt_sender_buffer.size())
                break;
